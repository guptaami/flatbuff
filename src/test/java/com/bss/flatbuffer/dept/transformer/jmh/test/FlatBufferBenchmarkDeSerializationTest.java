 ...
 *
 */
public class FlatBufferBenchmarkDeSerializationTest {
	...
	public static class BenchmarkState {
		...
		public void init() {
			...
			Department department = TestDataProvider.createTestData(departmentId, departmentName, tag,
					Genre.Educational, Subject.Maths);
			departmentStream = this.flatBuffertransformer.serialize(department);
		}
	}

	@Benchmark
	public void deSerialization(final BenchmarkState state, final Blackhole blackhole) {
		state.flatBuffertransformer.deserialize(state.departmentStream);
	}
}
